...
use crate::{
    ...
    models::{game::CreateGame, score::CreateScore},
};
...
pub async fn seed_from_csv(
    ...
    let game_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM game WHERE deleted_at IS NULL")
        .fetch_one(pool)
        .await?;

    if game_count > 0 {
        tracing::info!(
            "Database already contains {} games, skipping seed",
            game_count
        );
        return Ok(());
    }
    ...
    for (row_num, row) in rows.iter().enumerate() {
        ...
        let game_created_at = match DateTime::parse_from_rfc3339(&row.game_created_at) {
            ...
            Err(e) => {
                tracing::warn!(
                    "Row {}: Invalid game timestamp '{}': {}, skipping",
                    row_num + 1,
                    row.game_created_at,
                    e
                );
        ...
        if !games_map.contains_key(&normalized_hex_id) {
            ...
            games_map.insert(normalized_hex_id.clone(), (create_game, game_created_at));
        }
                ...
                let score_submitted_at = if let Some(ref submitted_str) = row.score_submitted_at {
                    if !submitted_str.is_empty() {
                        match DateTime::parse_from_rfc3339(submitted_str) {
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "Row {}: Invalid score timestamp '{}': {}, using current time",
                                    row_num + 1,
                                    submitted_str,
                                    e
                                );
    ...
    for (hex_id, (create_game, created_at)) in games_map {
        match GameRepository::create_with_hex_id(
            pool,
            create_game.clone(),
            hex_id.clone(),
            created_at,
        ...
        .await
        {
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to create game '{}' ({}): {}, continuing with remaining games",
                    create_game.name,
                    hex_id,
                    e
                );
    ...
    for (create_score, game_hex_id, submitted_at) in scores {
        match ScoreRepository::create_with_timestamp(pool, create_score.clone(), submitted_at).await
        {
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to create score '{}' for game {}: {}, continuing with remaining scores",
                    create_score.user_name,
                    game_hex_id,
                    e
                );
                failed_scores += 1;
...
pub async fn check_and_seed(pool: &DbPool) -> Result<(), Box<dyn std::error::Error>> {
    let seed_file =
        std::env::var("LEADR_SEED_FILE").unwrap_or_else(|_| "/data/seed.csv".to_string());
